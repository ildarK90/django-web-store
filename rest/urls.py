...
urlpatterns = [
    path('products_list/<str:slug>', ProductList.as_view(), name='cat_prod'),  # Вывод товаров без сериалайзера
    path('products/<str:model>', GeneralViewSet.as_view({'get': 'list'}), name='category_product'),
    # выполнено с помощью динамического сериалайзера
    path('latest_products', LatestProductsList.as_view()),
    # Выполненно  с помощью библиотеки для нескольких сериалайзеров
